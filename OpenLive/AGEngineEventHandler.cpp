...
{
	PAGE_PUBSLISHED_PARAM param = new AGE_PUBSLISHED_PARAM;
	int len = strlen(url);
	param->url = new char[len + 1];
	param->url[len] = 0;
...
{
	int len = strlen(url);
	char* rtmp_url = new char[len + 1];
	rtmp_url[len] = 0;
	strcpy_s(rtmp_url, len + 1, url);
	if (m_hMainWnd != NULL)
		::PostMessage(m_hMainWnd, WM_MSGID(EID_RTM_STREAM_UNPUBLISHED), (WPARAM)rtmp_url, 0);
}