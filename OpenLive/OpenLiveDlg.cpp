...
BOOL COpenLiveDlg::OnInitDialog()
{
	...
	if (strAppID.GetString() == 0 || strAppID.IsEmpty()) {
...
void COpenLiveDlg::InitCtrls()
{
	...
	m_linkAgora.SetURL(_T("http://home.qutoutiao.net"));
...
LRESULT COpenLiveDlg::OnJoinChannel(WPARAM wParam, LPARAM lParam)
{
	...
	if (m_dlgEnterChannel.bSelfDefineVideoProfile){
		m_dlgEnterChannel.GetSelfDefine(m_dlgVideo.transcodingWidth, m_dlgVideo.transcodingHeight, m_dlgVideo.transcodingfps, m_dlgVideo.transcodingBitrate);
		IRtcEngine2* rtcEngine2 = (IRtcEngine2*)lpRtcEngine;
		rtcEngine2->setVideoProfileEx(m_dlgVideo.transcodingWidth, m_dlgVideo.transcodingHeight, m_dlgVideo.transcodingfps, m_dlgVideo.transcodingBitrate);
	}
	else{
		int nVideoSolution = m_dlgSetup.GetVideoSolution();
		lpRtcEngine->setVideoProfile((VIDEO_PROFILE_TYPE)nVideoSolution, false);
		m_dlgVideo.transcodingWidth = m_dlgSetup.m_mapVideoProfile[(VIDEO_PROFILE_TYPE)nVideoSolution].width;
		m_dlgVideo.transcodingHeight = m_dlgSetup.m_mapVideoProfile[(VIDEO_PROFILE_TYPE)nVideoSolution].height;
		m_dlgVideo.transcodingfps = m_dlgSetup.m_mapVideoProfile[(VIDEO_PROFILE_TYPE)nVideoSolution].fps;
		m_dlgVideo.transcodingBitrate = m_dlgSetup.m_mapVideoProfile[(VIDEO_PROFILE_TYPE)nVideoSolution].bitrate;
	}
...
LRESULT COpenLiveDlg::OnSetPublishRtmp(WPARAM wParam, LPARAM lParam)
{
	...
	if (IDOK == publishdlg.DoModal()){
		...
		m_dlgVideo.m_rtmp_url = m_rtmp_url;
